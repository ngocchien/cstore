...
class storageStandProduct extends AbstractTableGateway {

    protected $table = 'tbl_stands_product';
    protected $adapter;
    ...
    public function getList($arrCondition = null) {
        try {
            ...
            $select = $sql->Select($this->table)
                    ->where('1=1' . $strWhere)
                    ->order(array('stand_prod_id DESC'));
...
//        p($p_arrParams);die;
        ...
        try {
            $adapter = $this->adapter;
            $sql = new Sql($adapter);
            $query = $sql->update($this->table)->set($p_arrParams)->where('1=1 AND stan_prod_id = ' . $intStandID);
...
//            p($query);die;
    ...
    private function _buildWhere($arrCondition) {
        ...
        if ($arrCondition['stan_prod_status'] !== '' && $arrCondition['stan_prod_status'] !== NULL) {
            $strWhere .= " AND stan_prod_status=" . $arrCondition['stan_prod_status'];
        }
