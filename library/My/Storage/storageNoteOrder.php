...
class storageNoteOrder extends AbstractTableGateway {

    protected $table = 'tbl_note_logs';
    protected $adapter;
...
//            p($p_arrParams);die;
    ...
    public function edit($p_arrParams, $intOrderID) {
        ...
        try {
            $adapter = $this->adapter;
            $sql = new Sql($adapter);
            $query = $sql->update($this->table)->set($p_arrParams)->where('1=1 AND note_id = ' . $intOrderID);
    ...
    public function delNoteOrder($intOrderID) {
        ...
        try {
            $adapter = $this->adapter;
            $sql = new Sql($adapter);
            $query = $sql->delete($this->table)->where('1=1 AND note_id = ' . $intOrderID);
            $query = $sql->getSqlStringForSqlObject($query);
            $result = $adapter->query($query, $adapter::QUERY_MODE_EXECUTE);
            $resultSet = new \Zend\Db\ResultSet\ResultSet();
            $resultSet->initialize($result);
    ...
    private function _buildWhere($arrCondition) {
        ...
        if (isset($arrCondition['note_id'])) {
            $strWhere .= " AND note_id=" . $arrCondition['note_id'];
        }
        if (isset($arrCondition['note_type'])) {
            $strWhere .= " AND note_type=" . $arrCondition['note_type'];
        }
        if (isset($arrCondition['note_position'])) {
            $strWhere .= " AND note_position LIKE '%" . $arrCondition['note_position'] . "%'";
        }
         if (isset($arrCondition['date_from'])) {
            $strWhere .= " AND note_created >=" . $arrCondition['date_from'];
        }
        if (isset($arrCondition['date_to'])) {
            $strWhere .= " AND note_created <=" . $arrCondition['date_to'];
        }
        return $strWhere;
    }
