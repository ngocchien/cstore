...
class Shipping extends ModelAbstract {
    ...
    private function getParentTable() {
        ...
        return new \My\Storage\storageShipping($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpShipping');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition, $intPage, $intLimit, $strShipping) {
        $keyCaching = 'getListLimitShipping:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strShipping) . ':' . $this->cache->read($this->tmpKeyCache);
