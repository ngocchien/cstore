...
class TagsContent extends ModelAbstract {

    private function getParentTable() {
        $dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
        return new \My\Storage\storageTagsContent($dbAdapter);
    }
    
    public function __construct() {
        $this->setTmpKeyCache('tmpTagsContent');
        parent::__construct();
    }
    ...
    public function getListUnlike($arrCondition = array()) {
        return $this->getParentTable()->getListUnlike($arrCondition,$intTagsID);
    }
    ...
    public function getListLimit($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'tags_content_id DESC') {
        $keyCaching = 'getListLimitTagContent:';
    ...
    public function getDetail($arrCondition = array()) {
        ...
        if ($arrCondition && is_array($arrCondition)) {
            $keyCaching = 'getDetailTagContent:';
            foreach ($arrCondition as $k => $condition) {
                $keyCaching .= $k . ':' . $condition;
            }
