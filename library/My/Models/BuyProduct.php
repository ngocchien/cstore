...
class BuyProduct extends ModelAbstract {

    private function getParentTable() {
        $dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
        return new \My\Storage\storageBuyProduct($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpBuyProduct');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition, $intPage, $intLimit, $strOrder) {
        $keyCaching = 'getListLimitBuyProduct:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function getDetail($arrCondition) {
        $keyCaching = 'getDetailBuyProduct:';
        if (count($arrCondition) > 0) {
