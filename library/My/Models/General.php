...
class General extends ModelAbstract {
    ...
    public function __construct() {
        $this->setTmpKeyCache('tmpGenerals');
        parent::__construct();
    }
    ...
    public function getLimit($arrCondition, $intPage, $intLimit, $strOrder) {
        $keyCaching = 'getLimitGeneral:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function edit($p_arrParams, $intGeneralID) {
        $ttl = 60 * 60 * 24 * 7;
        $intResult = $this->getParentTable()->edit($p_arrParams, $intGeneralID);
        if ($intResult) {
            $this->cache->increase($this->tmpKeyCache, 1);
        }
        return $intResult;
