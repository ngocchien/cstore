...
class OrderLog extends ModelAbstract {

    private function getParentTable() {
        $dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
        return new \My\Storage\storageOrderLog($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpOrderLog');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'orde_id DESC') {
        $keyCaching = 'getListLimitOrderLog:';
