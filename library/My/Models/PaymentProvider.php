...
class PaymentProvider extends ModelAbstract {

    private function getParentTable() {
        $dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
        return new \My\Storage\storagePaymentProvider($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpPaymentProvider');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'prov_id DESC') {
        $keyCaching = 'getListLimitPaymentProvider:';
    ...
    public function getDetail($arrCondition = array()) {
        ...
        if ($arrCondition && is_array($arrCondition)) {
            $keyCaching = 'getDetailPaymentProvider:';
            foreach ($arrCondition as $k => $condition) {
                $keyCaching .= $k . ':' . $condition;
            }
