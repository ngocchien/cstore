...
class StandProduct extends ModelAbstract {

    private function getParentTable() {
        $dbAdapter = $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter');
        return new \My\Storage\storageStandProduct($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpStandProduct');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'stan_prod_id DESC') {
        $keyCaching = 'getListLimitStandProduct:';
    ...
    public function getDetail($arrCondition = array()) {
        ...
        if ($arrCondition && is_array($arrCondition)) {
            $keyCaching = 'getDetailStandProduct:';
            foreach ($arrCondition as $k => $condition) {
                $keyCaching .= $k . ':' . $condition;
            }
    ...
    public function statisticOrder($arrCondition, $intPage, $intLimit, $strOrder = null) {
        $keyCaching = 'getListLimitOrderStatisticStandProduct:';
        foreach ($arrCondition as $k => $condition) {
            $keyCaching .= $k . ':' . $condition . ':';
        }
        $keyCaching .= $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function getUserBought($arrCondition = array(), $intPage = 1, $intLimit = 15, $strOrder = 'stan_prod_id DESC') {
