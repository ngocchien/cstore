...
class Menu extends ModelAbstract {
    ...
    private function getParentTable() {
        ...
        return new \My\Storage\storageMenu($dbAdapter);
    }

    public function __construct() {
        $this->setTmpKeyCache('tmpMenu');
        parent::__construct();
    }
    ...
    public function getListLimit($arrCondition, $intPage, $intLimit, $strOrder) {
        $keyCaching = 'getListLimitMenu:' . $intPage . ':' . $intLimit . ':' . str_replace(' ', '_', $strOrder) . ':' . $this->cache->read($this->tmpKeyCache);
    ...
    public function edit($p_arrParams, $intMenuID) {
        $ttl = 60 * 60 * 24 * 7;
        $intResult = $this->getParentTable()->edit($p_arrParams, $intMenuID);
        if ($intResult) {
            $this->cache->increase($this->tmpKeyCache, 1);
        }
        return $intResult;
